# ATIMONLAM 5.1 sound with headphones
Instructions on how to hear ATIMONLAM composition in 5.1 sound with headphones.

1. Purchase the track https://www.shangshungpublications.com/en/products/product/atimonlam-english (also available in other languages)

2. As a part of the package you'll find ATIMONLAM_5.1.wav file

3. Download and install Reaper (https://www.reaper.fm/download.php)

4. Download and install DearVR Monitor plugin (https://mega.nz/file/q5BRQaKK#DeRiKDaHlaPMpUS1Ask1eCVim56q4OjhzU2lMLevHHg), this plugin was made free to use by Sehnheiser

5. Open Reaper and drag ATIMONLAM_5.1.wav to the main window:

<img width="699" height="204" alt="image" src="https://github.com/user-attachments/assets/ba0e719b-8e21-47d2-898a-9993ef1416fb" />

6. Add DearVR Monitor through FX button on the left

7. Arrange settings as you see on this screenshot:

<img width="1316" height="732" alt="image" src="https://github.com/user-attachments/assets/2c6d9b6f-9f42-45a0-aeb7-549a9b2379cc" />

(you can also experiment with them if you want)

8. Hit play button and enjoy!!! ATIMONLAM in 5.1 sounds absolutely amazing!

<img width="213" height="50" alt="image" src="https://github.com/user-attachments/assets/73b30014-a585-422c-9c41-23a5b206ff19" />

